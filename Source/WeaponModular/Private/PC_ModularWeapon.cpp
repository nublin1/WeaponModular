...
void APC_ModularWeapon::SpawnInventorySceneRenderers(FWeaponData* WData)
{
	...
	FVector InstanceSpawnLocation = FVector(0.0f, 0.0f + IndexSpawn* 35.0f, 0.0f); 
	...
	RendererInstance->CaptureComponent->SetWorldLocation(FVector(-50.0f, 0.0f, 0.0f));
	...
	if(!InventoryItemSlotWidgetClass)
		return;
	TObjectPtr<UInventoryItemSlotWidget> InventoryItemSlotWidget = CreateWidget<UInventoryItemSlotWidget>(GetWorld(), InventoryItemSlotWidgetClass);
